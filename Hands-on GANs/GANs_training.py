...
class Discriminator(nn.Module):
    ...
    def forward(self, img, labels):
        ...
        return validity
# 儲存模型的函數
def save_checkpoint(generator, discriminator, epoch, path="model_checkpoint.pth"):
    torch.save({
        'epoch': epoch,
...
def load_checkpoint(generator, discriminator, optimizer_G, optimizer_D, path="model_checkpoint.pth"):
...
if __name__ == '__main__':
    ...
    checkpoint_path = "checkpoints/model_checkpoint.pth"  # 檢查點路徑
    if os.path.exists(checkpoint_path):
        start_epoch = load_checkpoint(generator, discriminator, optimizer_G, optimizer_D, path=checkpoint_path)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if epoch % 10 == 0:
            ...
            save_checkpoint(generator, discriminator, epoch, path=f"checkpoints/epoch_{epoch}.pth")
            with torch.no_grad():
                num_imgs = 64
                nrow = 8
